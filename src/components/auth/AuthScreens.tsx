...
export const AuthScreens: React.FC<AuthProps> = ({
  ...
  const handleRegister = (e: React.FormEvent) => {
    ...
    const newUser: User = {
      ...
      name: regName.trim(),
      email: email,
      password: 'DSLNG#2026', // Default password per specification
      department: regDepartment,
      work_location: regWorkLocation,
      role: 'user', // Default role is user per specification
    ...
    notifySuccess('Akun berhasil didaftarkan! Silakan Login dengan akun baru Anda.');
    setLoginEmail(email);
    setLoginPassword('DSLNG#2026');
    setAuthMode('login');
  };
          ...
          {authMode === 'login' && (
            <form onSubmit={handleLogin} className="space-y-4 sm:space-y-5">
              ...
              <div className="bg-sky-50 border border-sky-200 rounded-xl p-3 text-xs text-sky-900 flex items-start gap-2.5">
                <AlertCircle className="w-4 h-4 text-[#00A3E0] flex-shrink-0 mt-0.5" />
                <div className="leading-relaxed">
                  <span className="font-bold">Info Password Baru:</span> Akun yang baru didaftarkan memiliki default password{' '}
                  <code className="bg-white px-1.5 py-0.5 rounded border border-sky-300 font-mono font-bold text-[#004380]">
                    DSLNG#2026
                  </code>
                </div>
              </div>
          ...
          {authMode === 'register' && (
            <form onSubmit={handleRegister} className="space-y-4">
              ...
              <div className="bg-slate-50 border border-slate-200 rounded-xl p-3 text-xs text-slate-600 flex items-center gap-2">
                ...
                <span>
                  Password akun baru otomatis disetel ke: <span className="font-mono font-bold text-slate-900">DSLNG#2026</span>
                </span>
              </div>
